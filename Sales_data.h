...
public:
	...
	Sales_data(const string &s,const unsigned n,const double price)
		:bookNo(s),sold(n),revenue(price*n) {cout<<"111"<<endl;}
	// Sales_data(istream &is){
		// read(is,*this);
	// }
	Sales_data():Sales_data("",0,0.0) {cout<<"Sales_data()"<<endl;}
	Sales_data(const string &s):Sales_data(s,0,0.0) {cout<<"Sales_data(const string *s)"<<endl;}
	Sales_data(istream &is):Sales_data(){cout<<"222"<<endl;read(is,*this);}
	string isbn() const{
		return bookNo;
	}
